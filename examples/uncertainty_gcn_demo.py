...
config = UncertaintyGCNConfig(
    ...
    n_heads=2                           # Heads for mean and variance
)
...
key, subkey = jax.random.split(key)
params = model.init(subkey, atom_features, adjacency_matrix)

# 5. Make a prediction with uncertainty
mean, variance = model.apply(params, atom_features, adjacency_matrix)
...
for scale in scaling_factors:
    ...
    mean, var = model.apply(params, scaled_features, adjacency_matrix)
    means.append(mean[0])
...
plt.legend()
plt.grid(True)
plt.savefig('uncertainty_demo.png')
plt.close()
