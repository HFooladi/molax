...
def uncertainty_sampling(
    ...
    Args:
        ...
        pool_data: List of tuples containing (features, adjacency) matrices for each molecule
                  in the unlabeled pool. Features should be of shape (n_nodes, n_features)
                  and adjacency matrices should be of shape (n_nodes, n_nodes)
        n_samples: Number of MC samples to use for uncertainty estimation. Higher values
                  provide more accurate uncertainty estimates but increase computation time
    
    Returns:
        jnp.ndarray: Array of shape (n_pool,) containing uncertainty scores for each molecule
                    in the pool. Higher scores indicate higher uncertainty.
    """
    uncertainties = []
...
def diversity_sampling(
    ...
    Args:
        ...
        labeled_data: List of tuples containing (features, adjacency) matrices for each molecule
                     in the labeled set
        n_select: Number of samples to select from the pool
    
    Returns:
        List[int]: List of indices corresponding to the selected molecules in the pool_data list.
                  The length of the returned list will be n_select.
    """
...
def combined_acquisition(
    ...
    Args:
        ...
        uncertainty_weight: Float between 0 and 1 indicating the relative importance of
                           uncertainty vs diversity. A value of 1.0 means pure uncertainty
                           sampling, while 0.0 means pure diversity sampling.
